...
final class PostViewModel: ObservableObject {
    ...
    // MARK: - Real-time Listener (화면 진입 즉시 최신 글 수신)
    ...
    func startListening() {
        ...
        listener = db.collection("posts")
            ...
            .addSnapshotListener { [weak self] snap, err in
                Task { @MainActor in
                    if let err { self?.error = err.localizedDescription; self?.isLoading = false; return }
                    self?.posts = (snap?.documents ?? []).compactMap { try? $0.data(as: CommunityPost.self) }
    ...
    func addComment(postID: String, content: String, author: NormiUser) async {
        ...
        } catch { self.error = "댓글 등록에 실패했습니다." }
    }

    // MARK: - Delete Post

    func deletePost(_ post: CommunityPost, uid: String) {
        guard let id = post.id, post.authorUID == uid else { return }
