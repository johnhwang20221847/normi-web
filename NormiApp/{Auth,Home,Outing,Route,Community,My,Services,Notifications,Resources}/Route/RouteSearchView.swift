...
struct RouteSearchView: View {
                ...
                VStack(spacing: 0) {
                    ...
                    if vm.isLoadingRoute {
                        ...
                        VStack(spacing: 14) {
                            ...
                            Text("최적 경로를 탐색하는 중...").font(.normiBody).foregroundColor(.normiSubText)
                        }
                        Spacer()
    ...
    private var searchPanel: some View {
        VStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                stationField(icon: "circle.fill", color: .normiGreen,
                             placeholder: "출발역", text: $vm.depText) {
                    Task { await vm.searchDep() }
                }
                if !vm.depResults.isEmpty { autocompleteList(vm.depResults) { vm.selectDep($0) } }
            }

            // 스왑 + 구분
            HStack {
                Rectangle().fill(Color.normiGray).frame(height: 1)
            ...
            // 도착 필드
            VStack(alignment: .leading, spacing: 4) {
                stationField(icon: "mappin.circle.fill", color: .normiRed,
                             placeholder: "도착역", text: $vm.arrText) {
                    Task { await vm.searchArr() }
                }
                if !vm.arrResults.isEmpty { autocompleteList(vm.arrResults) { vm.selectArr($0) } }
            }

            // 탐색 버튼
            Button(action: { Task { await vm.findRoute() } }) {
                HStack { Image(systemName: "magnifyingglass"); Text("경로 탐색") }
            }
            ...
            if let err = vm.routeError {
                Label(err, systemImage: "exclamationmark.triangle.fill")
                    .font(.normiCaption).foregroundColor(.normiRed)
            }
        }
    }

    private func stationField(icon: String, color: Color, placeholder: String,
                               text: Binding<String>, onChange: @escaping () -> Void) -> some View {
        HStack(spacing: 10) {
            Image(systemName: icon).foregroundColor(color)
            TextField(placeholder, text: text).onChange(of: text.wrappedValue) { _ in onChange() }
        }
        .padding(12)
    ...
    private func autocompleteList(_ results: [(String, CLLocationCoordinate2D)],
                                   onSelect: @escaping ((String, CLLocationCoordinate2D)) -> Void) -> some View {
        VStack(spacing: 0) {
            ForEach(results.prefix(5), id: \.0) { r in
                Button(action: { onSelect(r) }) {
                    HStack {
                        Image(systemName: "tram").foregroundColor(.normiSubText)
                        Text(r.0).foregroundColor(.normiText)
                        Spacer()
                    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("경로 \(rank)")
                    ...
                    .background(rank == 1 ? Color.normiBlue : Color.normiGray)
                    .foregroundColor(rank == 1 ? .white : .normiSubText)
                    .cornerRadius(6)
                Spacer()
                VStack(alignment: .trailing, spacing: 2) {
                    Text(route.timeText).font(.normiTitle2).foregroundColor(.normiNavy)
                    Text(route.fareText).font(.normiCaption).foregroundColor(.normiSubText)
                }
            }
            // 노선 배지
            HStack(spacing: 6) {
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            Spacer()
            Image(systemName: "tram.circle").font(.system(size: 70)).foregroundColor(.normiBlue.opacity(0.25))
            Text("출발역과 도착역을 입력하고\n경로를 탐색해 보세요")
